...
export default function PrivacidadPage({ locale }: Props) {
  return (
    <>
      ...
      <Container className="py-10">
        <div className="mx-auto max-w-3xl space-y-6 text-papal-navy/85">
          ...
          {/* Aviso de idioma (solo para locales no-ES) */}
          {locale !== 'es' && (
            <div className="rounded-xl border border-amber-300 bg-amber-50 p-3 text-xs text-amber-900">
              {locale === 'en' && <>This page is primarily in Spanish. For GDPR rights or any legal question, please use the <Link href={localizePath('/contacto', locale)} className="font-bold underline">contact form</Link> — we reply in English.</>}
              {locale === 'it' && <>Questa pagina è principalmente in spagnolo. Per qualsiasi questione GDPR usa il <Link href={localizePath('/contacto', locale)} className="font-bold underline">modulo di contatto</Link>.</>}
              {locale === 'fr' && <>Cette page est principalement en espagnol. Pour toute question RGPD, utilise le <Link href={localizePath('/contacto', locale)} className="font-bold underline">formulaire de contact</Link>.</>}
              {locale === 'de' && <>Diese Seite ist hauptsächlich auf Spanisch. Für DSGVO-Fragen nutze das <Link href={localizePath('/contacto', locale)} className="font-bold underline">Kontaktformular</Link>.</>}
              {locale === 'pt' && <>Esta página está principalmente em espanhol. Para questões RGPD usa o <Link href={localizePath('/contacto', locale)} className="font-bold underline">formulário de contacto</Link>.</>}
              {locale === 'ca' && <>Aquesta pàgina és principalment en espanyol. Per qüestions RGPD usa el <Link href={localizePath('/contacto', locale)} className="font-bold underline">formulari de contacte</Link>.</>}
              {locale === 'gl' && <>Esta páxina está principalmente en castelán. Para cuestións RGPD usa o <Link href={localizePath('/contacto', locale)} className="font-bold underline">formulario de contacto</Link>.</>}
              {locale === 'eu' && <>Orri hau batez ere gaztelaniaz dago. RGPD galderentzat erabili <Link href={localizePath('/contacto', locale)} className="font-bold underline">kontaktu-formularioa</Link>.</>}
            </div>
          )}
              ...
              <table className="w-full border-collapse text-xs">
                ...
                <tbody>
                  <tr>
                    ...
                    <td className="border border-papal-gold/20 p-2">
                      Envío voluntario a través de <a href={localizePath('/contacto', locale)} className="text-papal-gold-dark hover:underline">/contacto</a>
                    </td>
                    <td className="border border-papal-gold/20 p-2">
          ...
          <section>
            ...
            <p className="mt-3 text-sm leading-relaxed">
              ...
              <Link href={localizePath('/contacto', locale)} className="text-papal-gold-dark hover:underline">formulario de contacto</Link>{' '}
          ...
          <section className="rounded-xl bg-papal-cream p-6 text-center">
            <p className="text-sm text-papal-navy/70">
              ...
              <Link href={localizePath('/politica-cookies', locale)} className="font-bold text-papal-gold-dark hover:underline">
                política de cookies
              </Link>{' '}
              y el{' '}
              <Link href={localizePath('/aviso-legal', locale)} className="font-bold text-papal-gold-dark hover:underline">
                aviso legal
              </Link>
              .
            </p>

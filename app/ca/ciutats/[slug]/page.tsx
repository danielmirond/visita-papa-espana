...
export function generateMetadata({ params }: Props): Metadata {
  ...
  if (!city) return {}
  const localized = getCitiesByLocale("ca").find((c) => c.slug === params.slug)
  const cityName = localized?.name || city.name
  const start = new Date(city.papalDates.start)
  const end = new Date(city.papalDates.end)
  const fmt = (d: Date) => d.toLocaleDateString("ca-ES", { day: "numeric", month: "long" })
  ...
  return {
    ...
    alternates: getAlternates(`/ciudades/${params.slug}`, "ca"),
    openGraph: {
...
export default function Page({ params }: Props) {
  return <CiudadDetailPage locale="ca" slug={params.slug} />
}

...
export function generateMetadata({ params }: Props): Metadata {
  ...
  if (!city) return {}
  const localized = getCitiesByLocale("de").find((c) => c.slug === params.slug)
  const cityName = localized?.name || city.name
  const start = new Date(city.papalDates.start)
  const end = new Date(city.papalDates.end)
  const fmt = (d: Date) => d.toLocaleDateString("de-DE", { day: "numeric", month: "long" })
  ...
  return {
    ...
    alternates: getAlternates(`/ciudades/${params.slug}`, "de"),
    openGraph: {
...
export default function Page({ params }: Props) {
  return <CiudadDetailPage locale="de" slug={params.slug} />
}
